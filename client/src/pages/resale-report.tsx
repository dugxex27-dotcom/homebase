...
import { Sparkles, Printer, RefreshCw, ArrowLeft, CheckSquare, AlertTriangle, ThumbsUp, Loader2, Info, Copy } from "lucide-react";
...
export default function ResaleReport() {
  ...
  const handleCopy = async () => {
    if (!report) return;
    const lines: string[] = [
      `RESALE READINESS REPORT — Grade: ${report.grade} (${GRADE_CONFIG[report.grade]?.label ?? ""})`,
      `Home Wellness Score: ${report.meta.wellnessScore}`,
      "",
      report.summary,
      "",
      "STRENGTHS",
      ...
      "",
      "CONCERNS",
      ...report.concerns.map((c, i) => `${i + 1}. ${c}`),
      "",
      "BEFORE YOU LIST — Action Plan",
      ...report.actionItems.map((a, i) => `${i + 1}. ${a}`),
      "",
      `Generated by MyHomeBase™ · ${new Date().toLocaleDateString()} · Not a formal appraisal.`,
    ];
    try {
      await navigator.clipboard.writeText(lines.join("\n"));
      toast({ title: "Report copied", description: "Paste it into an email or document." });
    } catch {
