...
function transactionStorage(client: DemoDb) {
  ...
  return {
    getUser: async (id: string) => first(client.select().from(users).where(eq(users.id, id)).limit(1)),
    ...
    createCrmLead: async (value: any) => first(client.insert(crmLeads).values(value).returning()),
    getCrmClient: async (id: string) => first(client.select().from(crmClients).where(eq(crmClients.id, id)).limit(1)),
    createCrmClient: async (value: any) => first(client.insert(crmClients).values(value).returning()),
    getCrmJob: async (id: string) => first(client.select().from(crmJobs).where(eq(crmJobs.id, id)).limit(1)),
    createCrmJob: async (value: any) => first(client.insert(crmJobs).values(value).returning()),
    getCrmQuote: async (id: string) => first(client.select().from(crmQuotes).where(eq(crmQuotes.id, id)).limit(1)),
    createCrmQuote: async (value: any) => first(client.insert(crmQuotes).values(value).returning()),
    getCrmInvoice: async (id: string) => first(client.select().from(crmInvoices).where(eq(crmInvoices.id, id)).limit(1)),
    createCrmInvoice: async (value: any) => first(client.insert(crmInvoices).values(value).returning()),
    getProposal: async (id: string) => first(client.select().from(proposals).where(eq(proposals.id, id)).limit(1)),
    createProposal: async (value: any) => first(client.insert(proposals).values(value).returning()),
  };
}
...
export async function ensureDemoAccountFlag<T extends { id: string; isDemoAccount?: boolean | null }>(
  ...
  if (user && user.isDemoAccount !== true) {
    await client.update(users).set({ isDemoAccount: true }).where(eq(users.id, user.id));
...
export async function seedHomeownerDemo(log: DemoLog, client: DemoDb = db): Promise<SeedOutcome> {
  ...
  const existingHouses = await demoStorage.getHouses(demoId);
  ...
  if (rogueHouses.length > 0) {
    ...
    for (const rogue of rogueHouses) {
      await client.delete(maintenanceLogs).where(eq(maintenanceLogs.houseId, rogue.id));
      await client.delete(taskOverrides).where(eq(taskOverrides.houseId, rogue.id));
      await client.delete(taskCompletions).where(eq(taskCompletions.houseId, rogue.id));
      await client.delete(homeSystems).where(eq(homeSystems.houseId, rogue.id));
      await client.delete(homeAppliances).where(eq(homeAppliances.houseId, rogue.id));
      await demoStorage.deleteHouse(rogue.id);
    }
  }

  const cleanedHouses = await demoStorage.getHouses(demoId);
  ...
  try {
    ...
    if (existingCount < DEMO_TASK_TARGET) {
      const houses = await demoStorage.getHouses(demoId);
...
export async function seedContractorDemo(log: DemoLog, client: DemoDb = db): Promise<SeedOutcome> {
  ...
  try {
    ...
    try {
      const leadSeed = [
        { id: "demo-lead-1", contractorUserId: demoId, companyId, firstName: "Michael", lastName: "Chen", email: "michael.chen@email.com", phone: "(206) 555-1234", projectType: "HVAC Repair", address: "1523 Pine Street, Seattle, WA 98101", status: "contacted", priority: "high", estimatedValue: "850.00", source: "website", notes: "AC not cooling. Customer has 20-year-old unit. Likely needs replacement. Scheduled estimate for next week.", followUpDate: new Date(Date.now() + 2 * 24 * 60 * 60 * 1000) },
        { id: "demo-lead-2", contractorUserId: demoId, companyId, firstName: "Jennifer", lastName: "Martinez", email: "jmartinez@email.com", phone: "(206) 555-5678", projectType: "Water Heater Installation", address: "892 Broadway Ave E, Seattle, WA 98102", status: "qualified", priority: "medium", estimatedValue: "1850.00", source: "referral", notes: "Current water heater is leaking. Wants tankless upgrade. Sent quote yesterday. Waiting for decision.", followUpDate: new Date(Date.now() + 5 * 24 * 60 * 60 * 1000) },
        { id: "demo-lead-3", contractorUserId: demoId, companyId, firstName: "Robert", lastName: "Thompson", email: "rthompson@email.com", phone: "(206) 555-9012", projectType: "Furnace Service", address: "3345 15th Ave W, Seattle, WA 98119", status: "new", priority: "medium", estimatedValue: "350.00", source: "other", notes: "Annual maintenance needed before winter. Called this morning. Need to call back to schedule.", followUpDate: new Date(Date.now() + 1 * 24 * 60 * 60 * 1000) },
    ...
    try {
      ...
      const existingChecks = await Promise.all(sampleHomeownerIds.map((id) => demoStorage.getUser(id)));
      if (!existingChecks.every((u) => u != null)) {
        for (let i = 0; i < sampleHomeownerIds.length; i++) {
          await demoStorage.upsertUser({
    ...
    try {
      ...
      const jobSeed = [
        ...
        { id: "demo-job-7", contractorUserId: demoId, companyId, clientId: "demo-client-4", title: "Rooftop Unit Compressor Replacement — Delgado Restaurant", description: "Replace failed compressor and restore cooling for the dining room rooftop unit.", serviceType: "Commercial HVAC", status: "completed", priority: "urgent", scheduledDate: new Date(nowMs - 24 * day), scheduledEndDate: new Date(nowMs - 24 * day + 7 * hr), actualStartTime: new Date(nowMs - 24 * day), actualEndTime: new Date(nowMs - 24 * day + 6.5 * hr), actualDuration: 390, estimatedDuration: 420, address: "7623 35th Ave SW", city: "Seattle", state: "WA", postalCode: "98126", laborCost: "1650.00", materialsCost: "2600.00", totalCost: "4250.00", completionNotes: "Compressor replaced, system evacuated and recharged. Supply temperature and amperage verified." },
        { id: "demo-job-8", contractorUserId: demoId, companyId, clientId: "demo-client-1", title: "High-Efficiency Furnace Installation — Patricia Nguyen", description: "Remove aging furnace and install a 96% AFUE variable-speed replacement.", serviceType: "HVAC Installation", status: "completed", priority: "normal", scheduledDate: new Date(nowMs - 36 * day), scheduledEndDate: new Date(nowMs - 36 * day + 8 * hr), actualStartTime: new Date(nowMs - 36 * day), actualEndTime: new Date(nowMs - 36 * day + 7.5 * hr), actualDuration: 450, estimatedDuration: 480, address: "4821 Fremont Ave N", city: "Seattle", state: "WA", postalCode: "98103", laborCost: "1480.00", materialsCost: "2200.00", totalCost: "3680.00", completionNotes: "New furnace commissioned, thermostat configured, and customer walkthrough completed." },
        { id: "demo-job-9", contractorUserId: demoId, companyId, clientId: "demo-client-2", title: "Whole-Home Water Softener Installation — Brian Okafor", description: "Install metered water softener with bypass valve and startup testing.", serviceType: "Plumbing", status: "completed", priority: "normal", scheduledDate: new Date(nowMs - 48 * day), scheduledEndDate: new Date(nowMs - 48 * day + 5 * hr), actualStartTime: new Date(nowMs - 48 * day), actualEndTime: new Date(nowMs - 48 * day + 4.5 * hr), actualDuration: 270, estimatedDuration: 300, address: "2207 10th Ave E", city: "Seattle", state: "WA", postalCode: "98102", laborCost: "795.00", materialsCost: "1400.00", totalCost: "2195.00", completionNotes: "Softener programmed for household usage. No leaks found after pressure test." },
        { id: "demo-job-10", contractorUserId: demoId, companyId, clientId: "demo-client-3", title: "Tankless Water Heater Conversion — Linda Cho", description: "Remove tank water heater and install condensing tankless system with gas-line upgrade.", serviceType: "Plumbing", status: "completed", priority: "high", scheduledDate: new Date(nowMs - 59 * day), scheduledEndDate: new Date(nowMs - 59 * day + 8 * hr), actualStartTime: new Date(nowMs - 59 * day), actualEndTime: new Date(nowMs - 59 * day + 7 * hr), actualDuration: 420, estimatedDuration: 480, address: "1108 NW 56th St", city: "Seattle", state: "WA", postalCode: "98107", laborCost: "1940.00", materialsCost: "3200.00", totalCost: "5140.00", completionNotes: "Tankless unit commissioned and recirculation schedule configured. Permit inspection passed." },
    ...
    try {
      ...
      const invoiceSeed = [
        ...
        { id: "demo-invoice-5", contractorUserId: demoId, companyId, clientId: "demo-client-4", jobId: "demo-job-7", invoiceNumber: "INV-DEMO-0005", title: "Rooftop Unit Compressor Replacement", status: "paid", lineItems: [{ description: "Commercial compressor replacement", quantity: 1, unitPrice: "4250.00", total: "4250.00" }], subtotal: "4250.00", taxRate: "0.00", taxAmount: "0.00", discount: "0.00", total: "4250.00", amountPaid: "4250.00", amountDue: "0.00", dueDate: new Date(nowMs - 17 * day), sentAt: new Date(nowMs - 23 * day), viewedAt: new Date(nowMs - 22 * day), paidAt: new Date(nowMs - 19 * day), paymentMethod: "bank_transfer" },
        { id: "demo-invoice-6", contractorUserId: demoId, companyId, clientId: "demo-client-1", jobId: "demo-job-8", invoiceNumber: "INV-DEMO-0006", title: "High-Efficiency Furnace Installation", status: "paid", lineItems: [{ description: "Furnace equipment and installation", quantity: 1, unitPrice: "3680.00", total: "3680.00" }], subtotal: "3680.00", taxRate: "0.00", taxAmount: "0.00", discount: "0.00", total: "3680.00", amountPaid: "3680.00", amountDue: "0.00", dueDate: new Date(nowMs - 29 * day), sentAt: new Date(nowMs - 35 * day), viewedAt: new Date(nowMs - 34 * day), paidAt: new Date(nowMs - 31 * day), paymentMethod: "credit_card" },
        { id: "demo-invoice-7", contractorUserId: demoId, companyId, clientId: "demo-client-2", jobId: "demo-job-9", invoiceNumber: "INV-DEMO-0007", title: "Whole-Home Water Softener Installation", status: "paid", lineItems: [{ description: "Water softener and installation", quantity: 1, unitPrice: "2195.00", total: "2195.00" }], subtotal: "2195.00", taxRate: "0.00", taxAmount: "0.00", discount: "0.00", total: "2195.00", amountPaid: "2195.00", amountDue: "0.00", dueDate: new Date(nowMs - 41 * day), sentAt: new Date(nowMs - 47 * day), viewedAt: new Date(nowMs - 46 * day), paidAt: new Date(nowMs - 43 * day), paymentMethod: "check" },
        { id: "demo-invoice-8", contractorUserId: demoId, companyId, clientId: "demo-client-3", jobId: "demo-job-10", invoiceNumber: "INV-DEMO-0008", title: "Tankless Water Heater Conversion", status: "paid", lineItems: [{ description: "Tankless water heater conversion", quantity: 1, unitPrice: "5140.00", total: "5140.00" }], subtotal: "5140.00", taxRate: "0.00", taxAmount: "0.00", discount: "0.00", total: "5140.00", amountPaid: "5140.00", amountDue: "0.00", dueDate: new Date(nowMs - 52 * day), sentAt: new Date(nowMs - 58 * day), viewedAt: new Date(nowMs - 57 * day), paidAt: new Date(nowMs - 54 * day), paymentMethod: "bank_transfer" },
