/**
 * HTTP-level integration tests: DELETE /api/appliances/:id must require
 * authentication and enforce house ownership, mirroring the pattern already
 * used by DELETE /api/home-systems/:id.
 *
 * Regression coverage for a previously-unauthenticated route: anyone who
 * guessed or enumerated an appliance ID could delete another homeowner's
...
vi.mock("../storage", () => ({
  storage: {
    ...
    deleteHomeAppliance: mockDeleteHomeAppliance,
...
describe("DELETE /api/appliances/:id — auth & ownership", () => {
  afterEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("returns 401 when no session is present (unauthenticated)", async () => {
    ...
    const res = await request(app).delete(`/api/appliances/${APPLIANCE_ID}`);

    expect(res.status).toBe(401);
    expect(mockDeleteHomeAppliance).not.toHaveBeenCalled();
  });
  ...
  it("deletes the appliance when the authenticated user owns the house", async () => {
    const app = await buildApp();
    mockGetHomeAppliance.mockResolvedValue({ id: APPLIANCE_ID, houseId: HOUSE_ID });
    mockGetHouse.mockResolvedValue({ id: HOUSE_ID, homeownerId: OWNER_ID });
    mockDeleteHomeAppliance.mockResolvedValue(true);
    ...
    expect(mockDeleteHomeAppliance).toHaveBeenCalledWith(APPLIANCE_ID);
  });
});
